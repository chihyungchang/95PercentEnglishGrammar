    ...
    >
      ...
      <div className="p-6 pl-14 h-full overflow-y-auto">
        ...
        <div className="text-center mb-5 pb-3 border-b-2 border-[#1a1a2e]">
          ...
          <p className="text-base text-gray-500 mt-1">主谓宾和它的兄弟们</p>
        </div>
        ...
        <Section num={3} title="主系表" en="S + Link.V + P">
          ...
          <div className="bg-[#fef9e7] border border-[#d4a017] rounded-lg p-3 mt-2">
            <p className="text-sm font-semibold text-[#d4a017] mb-2">表语可以是：</p>
            <div className="flex flex-wrap gap-2">
              {[['名词', 'noun'], ['形容词', 'adj.'], ['不定式', 'to do'], ['介宾', 'prep.+n.']].map(([zh, en]) => (
                <span key={zh} className="text-sm bg-white border border-gray-200 px-2 py-1 rounded">
        ...
        {/* 动词分类与系动词 */}
        <div className="mt-5">
          <h3 className="font-serif text-lg font-bold mb-3">动词分类与系动词</h3>
          <div className="flex gap-4">
            {/* Venn Diagram */}
            <div className="relative w-[150px] h-[150px] flex-shrink-0">
              {/* Main circle */}
              <div className="absolute w-[140px] h-[140px] top-0 left-[10px] border-2 border-[#1e8449] rounded-full bg-[#1e8449]/5">
                <span className="absolute top-2 right-8 text-sm font-semibold text-[#1e8449]">V 动词</span>
              </div>
              {/* Trans */}
              <div className="absolute w-12 h-12 top-9 left-4 border-2 border-[#2d5aa0] rounded-full bg-[#e8f0fe] flex items-center justify-center">
                <span className="text-sm font-semibold text-[#2d5aa0] text-center leading-tight">及物<br/>动词</span>
              </div>
              {/* Intrans */}
              <div className="absolute w-12 h-12 top-9 right-4 border-2 border-[#c0392b] rounded-full bg-[#fdeaea] flex items-center justify-center">
                <span className="text-sm font-semibold text-[#c0392b] text-center leading-tight">不及物<br/>动词</span>
              </div>
              {/* Linking */}
              <div className="absolute w-14 h-8 bottom-6 left-1/2 -translate-x-1/2 border-2 border-[#8e44ad] rounded-full bg-[#f5eef8] flex items-center justify-center">
                <span className="text-sm font-semibold text-[#8e44ad]">系动词</span>
              </div>
            </div>
        ...
        <div className="absolute bottom-4 right-6 font-mono text-sm text-gray-400">2</div>
      </div>
    </div>
  )
})
  ...
  return (
    <div className="bg-gray-50/90 border border-gray-200 rounded-lg p-3">
      ...
      <div className="text-center mt-2 pt-2 border-t border-dashed border-gray-200">
        <span className="font-mono text-base">{example}</span>
      </div>
      {note && <p className="text-sm text-gray-500 text-center mt-1.5 italic">{note}</p>}
    </div>
  )
}
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-2.5">
      <div className="flex items-center gap-1.5 mb-1.5">
        <span className="w-4 h-4 bg-[#8e44ad] text-white text-xs font-mono rounded-full flex items-center justify-center">
          {num}
        </span>
        <span className="font-semibold text-base">{title}</span>
      </div>
      <p className="font-mono text-sm text-[#8e44ad]">{verbs}</p>
    </div>
  )
}
...
const Plus = () => <span className="text-gray-400 font-mono text-base mx-1">+</span>
